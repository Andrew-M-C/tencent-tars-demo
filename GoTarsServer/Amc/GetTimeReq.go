...
type GetTimeReq struct {
	Time_fmt string `json:"time_fmt"`
}

func (st *GetTimeReq) resetDefault() {
}
...
func (st *GetTimeReq) ReadFrom(_is *codec.Reader) error {
	...
	err = _is.Read_string(&st.Time_fmt, 0, false)
	if err != nil {
		return err
	}
...
func (st *GetTimeReq) ReadBlock(_is *codec.Reader, tag byte, require bool) error {
	...
	if !have {
		if require {
			return fmt.Errorf("require GetTimeReq, but not exist. tag %d", tag)
		}
		return nil
...
func (st *GetTimeReq) WriteTo(_os *codec.Buffer) error {
	...
	err = _os.Write_string(st.Time_fmt, 0)
	if err != nil {
		return err
	}
